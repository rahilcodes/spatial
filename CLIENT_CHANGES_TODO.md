...
- [x] Round 5d (live-site crash): /news, /careers, /downloads showed "Application error" on Vercel because the database is unreachable there (instant failure = DATABASE_URL missing in Vercel env). Fixes shipped: all three pages now fall back to seed content with a logged error instead of crashing (`lib/fallback.ts`); new `/api/health` endpoint reports env-var presence (booleans only) and DB connectivity so the cause is provable after deploy. ACTION FOR RAHIL: add DATABASE_URL (and BLOB_READ_WRITE_TOKEN if missing) in Vercel → Settings → Environment Variables for Production + Preview, then redeploy; confirm at https://www.spatialalphabet.com/api/health.
- [x] Hyderabad office map: replaced the Koti coordinates with the client's Google Maps embed for "Spatial Alphabet .Int.Inc" (17.4585421, 78.3630982); coordinates text, directions link and QR code regenerated to match. Street address text still not supplied by client.
- [x] Hyderabad street address added (contact card, Who We Are dual-shore card, footer, Organization JSON-LD): B Block 609, 6th Floor, Asian Sun City, Kondapur, Forest Dept Colony, Gachibowli, Kothaguda, Hyderabad, Telangana 500084. Vercel env audit: only DATABASE_URL is missing — adding it fixes the live news/careers/downloads pages.
